...
export const SettingsModal = () => {
  ...
  const updateModelConfig = useWorkspaceStore((state) => state.updateModelConfig)
  const [activeTab, setActiveTab] = useState<SettingTab>('models')
  const [selectedProvider, setSelectedProvider] = useState<ModelProviderId>(activeProvider)
  ...
  const openAddProvider = () => {
    const nextProvider = modelProviders.find((provider) => !drafts[provider.id].enabled)?.id ?? modelProviders[0].id
  ...
  const handleSaveProvider = () => {
    ...
    updateDraft({
      ...
      activeModel: selectedConfig.activeModel || selectedMeta.defaultModel,
    })
    closeProviderForm()
  }
  ...
  const handleDeleteProvider = (provider: ModelProviderId) => {
    ...
    if (selectedProvider === provider) {
      const nextProvider = modelProviders.find((item) => item.id !== provider && drafts[item.id].enabled)
      setSelectedProvider(nextProvider?.id ?? provider)
    }
  }

  return (
    <div className="fixed inset-0 z-50 flex bg-background text-text" onWheel={(event) => event.stopPropagation()}>
            ...
            return (
              <button
                ...
                className={`flex h-8 w-full items-center gap-3 rounded-lg px-3 text-sm transition ${
                  active ? 'bg-surfaceHover text-text' : 'text-textMuted hover:bg-surface'
                }`}
        ...
        {activeTab === 'models' ? (
          <section className="mx-auto max-w-[1080px]">
            <div className="mb-5 flex items-end justify-between">
              <div>
                <h1 className="text-3xl font-semibold tracking-normal text-text">模型供应商</h1>
              ...
              <Space>
                ...
                <Button type="primary" icon={<Plus size={14} />} onClick={openAddProvider}>
                  添加模型
                </Button>
              </Space>
            </div>
              ...
              {configuredProviders.length > 0 ? (
                <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                  {configuredProviders.map((provider) => {
                    ...
                    const hiddenModelCount = Math.max(config.models.length - visibleModels.length, 0)
                      ...
                      >
                        ...
                        >
                          <div className="mb-5 flex items-start justify-between gap-4">
                            <h2 className="min-w-0 truncate text-base font-semibold text-text">{provider.name}</h2>
                            <Space size={8} className="shrink-0">
                              ...
                              <Tag className="m-0 !border-accent/20 !bg-accent/10 !text-accent">自定义</Tag>
                            </Space>
                          </div>
                          ...
                          <div className="grid grid-cols-[84px_minmax(0,1fr)] gap-y-3 text-sm">
                            ...
                            <span className="text-right text-xs text-textMuted">
                              {visibleModels.length}/{config.models.length} 个模型
                            </span>
                          </div>
                        ...
                        <div className="mt-6 flex items-center gap-5 border-t border-border pt-3 text-xs">
                          ...
                          >
                            <Button type="link" danger className="ml-auto h-auto p-0">
                              删除
                            </Button>
                          </Popconfirm>
                        </div>
                ...
                <Card>
                  <Empty description="暂无已配置模型供应商" image={Empty.PRESENTED_IMAGE_SIMPLE}>
                    <Button type="primary" icon={<Plus size={14} />} onClick={openAddProvider}>
                      添加模型
                    </Button>
                  </Empty>
          ...
          <section className="mx-auto max-w-[720px]">
            <h1 className="text-3xl font-semibold tracking-normal text-text">关于 Nova Desk</h1>
            <div className="mt-7 rounded-xl border border-border bg-card p-6 text-sm leading-7 text-textSecondary">
              <p>Nova Desk 是本地桌面工作台，用于在当前工作区内组织会话、模型供应商和 Agent 能力。</p>
      ...
      >
        <Form layout="vertical" className="pt-3">
          ...
          <Form.Item label="模型" required>
            <Select
              value={selectedConfig.activeModel}
              options={selectedProviderModelOptions}
              onChange={(activeModel: string) => updateDraft({ activeModel })}
            />
          </Form.Item>
        </Form>
      </Modal>
