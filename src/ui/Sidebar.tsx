import { Bot, Clock3, Folder, MessageSquarePlus, Search, Settings, SlidersHorizontal, Workflow } from 'lucide-react'
...
const projectItems = [
  ...
  { title: 'helixdr-manage-web', subtitle: '这个文件是用的 approval...', time: '2 天' },
  ...
  return (
    <aside className="flex min-h-screen flex-col border-r border-border bg-[#f0f1f3] text-[#2f343d]">
      <div className="flex items-center gap-3 px-4 pb-3 pt-4">
        <img src="/brand/icon.png" alt="Nova Desk" className="h-10 w-10 rounded-xl object-cover shadow-sm" draggable={false} />
      ...
      <div className="mt-7 min-h-0 flex-1 overflow-y-auto px-3">
        ...
        <div className="space-y-1">
          {chatItems.map((item) => (
            <ConversationItem key={item.title} icon={Bot} title={item.title} time={item.time} />
          ))}
        </div>
      </div>

      <div className="flex items-center justify-between border-t border-[#dfe3e8] px-4 py-3">
        <button type="button" className="inline-flex items-center gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-white/70">
          <Settings size={15} />
          设置
        </button>
        <button type="button" className="rounded-full bg-white px-3 py-1.5 text-xs shadow-sm hover:text-primary">
          升级
        </button>
      </div>
    </aside>
...
const ConversationItem = ({
  ...
  >
    ...
    <span className="min-w-0">
      ...
      {subtitle ? <span className="block truncate text-xs text-[#7e8490]">{subtitle}</span> : null}
    </span>
